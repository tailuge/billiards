...
`,ye=(n,e=!1)=>c`<a class="arena-item ${e?"completed":""}" href="arena.html?tournamentId=${encodeURIComponent(n.id)}">
...
</a>`,fe=class extends f{static properties={heading:{type:String},_arenas:{state:!0},_error:{state:!0}};static styles=[ve,b`
        :host { display: block; }
    ...
    `];constructor(){super(),this.heading="",this._arenas=[],this._error="",this._timer=null}connectedCallback(){super.connectedCallback(),this._load(),this._timer=setInterval(()=>this._load(),At)}disconnectedCallback(){this._timer&&(clearInterval(this._timer),this._timer=null),super.disconnectedCallback()}async load(){await this._load()}async _load(){try{let e=await fetch(`${ge}/api/arena`),t=await e.json();if(!e.ok)throw new Error(t.error||`Unable to load Arenas (${e.status})`);let i=Date.now();if(!(t.arenas||[]).some(r=>r.endTime>i&&r.status!=="finished")){let r=Pt(i);if(Math.floor((r-i)/6e4)>=kt){let l=new Date(r),o=String(l.getUTCHours()).padStart(2,"0"),d=String(l.getUTCMinutes()).padStart(2,"0"),h=`${l.getUTCFullYear()}${String(l.getUTCMonth()+1).padStart(2,"0")}${String(l.getUTCDate()).padStart(2,"0")}-${o}${d}`,p=ot[l.getUTCHours()%ot.length];if(await fetch(`${ge}/api/arena`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`arena-hourly-${h}`,creatorId:"hourly-arena",creatorName:p.name,ruleType:p.ruleType,options:p.options,endTime:r})}),e=await fetch(`${ge}/api/arena`),t=await e.json(),!e.ok)throw new Error(t.error||`Unable to reload Arenas (${e.status})`)}}this._arenas=(t.arenas||[]).sort((r,a)=>a.createdAt-r.createdAt),this._error="",this.dispatchEvent(new CustomEvent("arenas-loaded",{detail:{arenas:this._arenas},bubbles:!0,composed:!0}))}catch(e){this._error=e.message||"Unable to load Arenas."}}get _activeArenas(){let e=Date.now();return this._arenas.filter(t=>t.endTime>e&&t.status!=="finished")}render(){let e=this._activeArenas;return c`
            ...
            ${this._error&&!e.length?c`<div class="error">Could not load arenas.</div>`:e.length?c`<div class="arena-list" aria-label="Active Arenas">${e.map(t=>ye(t))}</div>`:c`<div class="empty">No active Arenas.</div>`}
