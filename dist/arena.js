...
`];var Ke=932,Xe=n=>`v${Math.floor(n/100)}.${String(n%100).padStart(2,"0")}`;var z=typeof localStorage<"u"&&localStorage.getItem("useProxy")==="true"?"nchanproxy.tailuge.workers.dev":"billiards-network.onrender.com",Ze=typeof window<"u"&&(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"),me=Ze?`ws://${window.location.hostname}:80`:`wss://${z}`;var G=typeof window<"u"&&window.location.hostname.includes("vercel");var wt=Ze?`http://${window.location.hostname}:8080/`:"https://billiards.tailuge.workers.dev/",qe=(n,e)=>e?Object.entries(e).reduce((t,[i,s])=>t+`&${encodeURIComponent(i)}=${encodeURIComponent(s)}`,n):n,et=(n,e,t)=>{for(let[i,s]of Object.entries(n)){let r=e?`${e}.${encodeURIComponent(i)}`:encodeURIComponent(i);s&&typeof s=="object"&&!Array.isArray(s)?et(s,r,t):s!=null&&t.push(`${r}=${encodeURIComponent(s)}`)}return t},Qe=(n,e,t)=>e&&typeof e=="object"?et(e,t,[]).reduce((i,s)=>i+`&${s}`,n):n;var V=({tableId:n,userId:e,userName:t,ruleType:i,isFirst:s,options:r,localOptions:a,bot:l,lod:o,flip:c,custom:u,opponent:m})=>{let h=`${wt}?websocketserver=${me}&userName=${encodeURIComponent(t)}&userId=${e}&ruletype=${i}`;return l||(h+=`&tableId=${n}`),s&&(h+="&first=true"),l&&(h+=`&bot=${encodeURIComponent(l)}`),o!==void 0&&(h+=`&lod=${o}`),c&&(h+="&flip=true"),h=qe(h,r),h=qe(h,a),h=Qe(h,u,"custom"),m?.userId&&(h+=`&opponent.userId=${encodeURIComponent(m.userId)}&opponent.userName=${encodeURIComponent(m.userName||"")}`,h=Qe(h,m.custom,"opponent.custom")),h};var $t={eightball:"eightball",snooker:"snooker",threecushion:"threecushion",nineball:"nineball",sagu:"sagu"},ge=n=>{let e=$t[n];return e?d`<img src="assets/${e}.png" alt="${n}" title="${n}" width="18" height="18" style="vertical-align:middle">`:d`🎱`},tt=(n,e={})=>d`<span title="${n}">
    ...
    `];constructor(){super(),this.arenaId="",this.theme=document.documentElement.getAttribute("theme")||localStorage.getItem("theme")||"dark",this._arena=null,this._leaderboard=[],this._onlineUsers=[],this._busy=!1,this._theme=this.theme,document.documentElement.setAttribute("theme",this._theme),document.documentElement.style.colorScheme=this._theme,this._presenceClient=null,this._lobby=null,this._error="",this._timer=null,this._staleRefetchDone=!1,this._lastLoadedArenaId=null,this._pairingState=null,this._pairingCountdown=$e,this._pairedName="",this._beserk=!1,this._pairingInterval=null,this._pairingTimeout=null,this._pendingArenaChallenge=null}connectedCallback(){super.connectedCallback(),this._load(),this._connectPresence(),this._timer=setInterval(()=>{this._arena&&this.requestUpdate()},1e3)}disconnectedCallback(){this._timer&&(clearInterval(this._timer),this._timer=null),this._cancelPairing(),this._pendingArenaChallenge=null,this._lobby?.leave(),this._presenceClient?.stop(),super.disconnectedCallback()}_isExpired(){return!!(this._arena&&this._arena.endTime&&Date.now()>=this._arena.endTime)}get _localCustom(){let e=p.getCustom();return(e.emoji===void 0||e.emoji===null)&&this._onlineUsers.find(i=>i.userId===p.clientId)?.meta?.country==="BOT"?{...e,emoji:"\u{1F916}"}:e}_getCountdownText(){if(!this._arena||!this._arena.endTime)return"";let e=Math.max(0,this._arena.endTime-Date.now());if(e<=0||this._arena.status==="finished")return"00:00";let t=Math.floor(e/1e3),i=Math.floor(t/60),s=t%60;if(i>=60){let r=Math.floor(i/60),a=i%60;return`${String(r).padStart(2,"0")}:${String(a).padStart(2,"0")}:${String(s).padStart(2,"0")}`}return`${String(i).padStart(2,"0")}:${String(s).padStart(2,"0")}`}async _connectPresence(){let e=window.location.protocol==="https:"?"https:":"http:",t=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?`${e}//${window.location.host}`:`https://${z}`;this._presenceClient=new X({baseUrl:t}),this._presenceClient.setVersion(Xe(Ke));try{this._lobby=await this._presenceClient.joinLobby({messageType:"presence",type:"join",userId:p.clientId,userName:p.userName}),await this._syncArenaPresence(),this._lobby.onUsersChange(i=>{this._onlineUsers=[...i,{userId:p.clientId,userName:p.userName,custom:this._localCustom}],this._checkStaleArenaPresence()}),this._lobby.onChallenge(i=>{i.type==="offer"?this._handleIncomingChallenge(i):this._handleArenaChallengeMessage(i)})}catch(i){console.error("Arena presence connection failed:",i)}}async _handleIncomingChallenge(e){if(e.type!=="offer"||e.challengeeId!==p.clientId)return;let t=this._pairingState==="counting";if(this._cancelPairing(),!this._arena?.players?.some(s=>s.playerId===p.clientId&&s.active!==!1)||e.options?.tournamentId!==this.arenaId){t&&this.requestUpdate();return}await this._acceptArenaChallenge(e)}_handleArenaChallengeMessage(e){let t=this._pendingArenaChallenge;if(!t||e.challengerId!==p.clientId||e.challengeeId!==t.opponentId)return;if(e.type==="decline"||e.type==="cancel"){this._pendingArenaChallenge=null;return}if(e.type!=="accept")return;if(!t.tableId){t.earlyAccept=e;return}if(e.tableId!==t.tableId)return;this._pendingArenaChallenge=null;let i=e.ruleType||t.ruleType,s=e.options||t.options,r=e.nextTurnId?e.nextTurnId===p.clientId:!0,a=V({tableId:e.tableId,userId:p.clientId,userName:p.userName,ruleType:i,isFirst:r,options:s,localOptions:this._beserk?{beserk:"true"}:void 0,lod:p.lod,flip:p.flip,custom:this._localCustom,opponent:{userId:t.opponentId,userName:t.opponentName,custom:t.opponentCustom}});window.location.href=a}async _acceptArenaChallenge(e){if(!this._lobby)return;let t=e.ruleType||this._arena?.ruleType||"nineball",i=e.options||this._arena?.options||{};try{await this._lobby.acceptChallenge(e.challengerId,t,e.tableId,i,e.challengerName,void 0,this._localCustom)}catch(a){console.error("Arena auto-accept failed:",a);return}let s=e.nextTurnId?e.nextTurnId===p.clientId:!1,r=V({tableId:e.tableId,userId:p.clientId,userName:p.userName,ruleType:t,isFirst:s,options:i,localOptions:this._beserk?{beserk:"true"}:void 0,lod:p.lod,flip:p.flip,custom:this._localCustom,opponent:{userId:e.challengerId,userName:e.challengerName||"",custom:e.custom}});window.location.href=r}async _load(){this._busy=!0,this._error="";try{let e=await fetch(`${at}/api/arena/${encodeURIComponent(this.arenaId)}`),t=await e.json();if(!e.ok)throw new Error(t.error||`Unable to load Arena (${e.status})`);this._arena=t.arena,this._leaderboard=t.leaderboard||[],this.arenaId!==this._lastLoadedArenaId&&(this._staleRefetchDone=!1,this._lastLoadedArenaId=this.arenaId),await this._syncArenaPresence(),this._checkStaleArenaPresence()}catch(e){this._error=e.message||"Unable to load Arena."}finally{this._busy=!1}}async _join(){let e=(p.userName||"").trim();if(/^(anonymous|anon)$/i.test(e)){window.alert("You must change name, Anonymous is not a valid arean name");return}await this._mutate("join",{playerId:p.clientId,name:e})}async _leave(){this._cancelPairing(),await this._mutate("leave",{playerId:p.clientId})}async _syncArenaPresence(){if(!this._lobby||!this._arena)return;let e=this._arena.players?.find(t=>t.playerId===p.clientId);try{await this._lobby.updatePresence({arenaId:e?.active!==!1&&e?this.arenaId:void 0})}catch(t){console.error("Failed to update Arena presence:",t)}}_checkStaleArenaPresence(){if(!this._arena||!this._lobby)return;this._onlineUsers.some(t=>t.userId!==p.clientId&&t.arenaId===this.arenaId&&!this._arena.players?.some(i=>i.playerId===t.userId))&&this._refetchStaleArenaOnce()}_refetchStaleArenaOnce(){this._staleRefetchDone||this._busy||(this._staleRefetchDone=!0,this._load())}async _mutate(e,t){this._busy=!0,this._error="";try{let i=await fetch(`${at}/api/arena/${encodeURIComponent(this.arenaId)}/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}),s=await i.json();if(!i.ok)throw new Error(s.error||`${e} failed (${i.status})`);await this._load()}catch(i){this._error=i.message||`Unable to ${e} Arena.`,this._busy=!1}}_getPairingCandidates(){let e=p.clientId,t=[],i=[],s=[];for(let r of this._leaderboard){if(r.playerId===e)continue;let a=we(r.playerId),l=this._arena?.players?.find(h=>h.playerId===r.playerId),o=this._onlineUsers.find(h=>h.userId===r.playerId),c=!a&&!!o?.tableId,u=l?.active!==!1&&(a||!!(o&&!c)),m={playerId:r.playerId,name:r.name,playing:c,bot:a,available:u};s.push(m),u&&(a?i.push(r):t.push(r))}return{candidates:t.length>0?t:i,diagnostics:s}}_findEligibleOpponents(){return this._getPairingCandidates().candidates}_startPairing(){this._pairingState!=="counting"&&(this._pairingState="counting",this._pairingCountdown=this._getPairingCandidates().candidates.some(e=>!we(e.playerId))?At:$e,this._pairedName="",this._pairingInterval=setInterval(()=>this._onPairingTick(),1e3))}_cancelPairing(){this._pairingInterval&&(clearInterval(this._pairingInterval),this._pairingInterval=null),this._pairingTimeout&&(clearTimeout(this._pairingTimeout),this._pairingTimeout=null),this._pairingState=null,this._pairingCountdown=$e,this._pairedName=""}_onPairingTick(){let e=this._arena,t=p.clientId,i=e?.players?.some(r=>r.playerId===t&&r.active!==!1),s=e?.status==="active"&&Date.now()<(e?.endTime||0);if(!i||!s){this._cancelPairing();return}if(this._pairingCountdown-=1,this._pairingCountdown<=0){clearInterval(this._pairingInterval),this._pairingInterval=null,this._executePairing();return}this.requestUpdate()}_getOpponentHistory(){if(!this.arenaId)return[];try{let e=localStorage.getItem(`arena_opponents_${this.arenaId}`);return e?JSON.parse(e):[]}catch{return[]}}_recordOpponentHistory(e){if(!(!this.arenaId||!e))try{let t=this._getOpponentHistory();t.push(e),t.length>10&&t.shift(),localStorage.setItem(`arena_opponents_${this.arenaId}`,JSON.stringify(t))}catch(t){console.error("Failed to save opponent history:",t)}}async _executePairing(){let{candidates:e,diagnostics:t}=this._getPairingCandidates();if(e.length===0){console.log("[Arena pairing]",{candidates:t,choice:null}),this._pairingState="no-opponent",this._pairingTimeout=setTimeout(()=>this._cancelPairing(),ot);return}let i=this._getOpponentHistory(),s={};for(let c of i)s[c]=(s[c]||0)+1;let r=1/0;for(let c of e){let u=s[c.playerId]||0;u<r&&(r=u)}let a=e.filter(c=>(s[c.playerId]||0)===r),l=a[Math.floor(Math.random()*a.length)];this._recordOpponentHistory(l.playerId);let o=t.find(c=>c.playerId===l.playerId);console.log("[Arena pairing]",{candidates:t,choice:o}),this._pairedName=l.name,this._pairingState="paired";try{await this._initiateChallenge(l)}catch(c){console.error("Pairing challenge failed:",c)}this._pairingTimeout=setTimeout(()=>this._cancelPairing(),ot)}async _initiateChallenge(e){let t=this._arena,i=t?.ruleType||"nineball",s=t?.options||{},r=t?.id||"";if(we(e.playerId)){let o=Tt[e.playerId]||e.name,c="arena-bot-"+Math.random().toString(36).slice(2,8),u=V({tableId:c,userId:p.clientId,userName:p.userName,ruleType:i,isFirst:!0,options:s,bot:o,lod:p.lod,custom:this._localCustom,opponent:{userId:e.playerId,userName:e.name,custom:e.custom},flip:p.flip,localOptions:this._beserk?{beserk:"true"}:void 0});window.location.href=u+`&tournamentId=${encodeURIComponent(r)}`;return}if(!this._lobby){console.error("Pairing: no lobby connection");return}let a={...s,tournamentId:r},l={opponentId:e.playerId,opponentName:e.name,opponentCustom:e.custom||this._onlineUsers.find(o=>o.userId===e.playerId)?.custom,ruleType:i,options:a,tableId:null,earlyAccept:null};this._pendingArenaChallenge=l;try{let o=await this._lobby.challenge(e.playerId,i,a,void 0,this._localCustom);if(l.tableId=o,l.earlyAccept){let c=l.earlyAccept;l.earlyAccept=null,this._handleArenaChallengeMessage(c)}}catch(o){throw this._pendingArenaChallenge===l&&(this._pendingArenaChallenge=null),o}}_renderPairingOverlay(){return this._pairingState==="counting"?d`
                ...
                </div>`:null}render(){let e=this._arena,t=p.clientId,i=e?.players?.find(m=>m.playerId===t),s=!!i,r=i?.active!==!1,a=this._isExpired(),l=e?.status==="active"&&!a,o=s&&r&&l&&this._pairingState===null,c=this._pairingState!==null,u=this._renderPairingOverlay();return d`<div class="container">
            ...
            ${e?d`
                <section class="panel">
                    ...
                    ${a?"":d`<div class="actions">
                        ...
                        ${o?d`<button class="btn-challenge" type="button" @click=${this._startPairing}>Pair</button><button class="btn-challenge" type="button" aria-pressed=${this._beserk} @click=${()=>{this._beserk=!this._beserk}}>Beserk 🚀</button>`:""}
                    </div>`}
                </section>
                <section class="panel">
                    ${u}
