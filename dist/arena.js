...
`,ve=(n,e=!1)=>c`<a class="arena-item ${e?"completed":""}" href="arena.html?tournamentId=${encodeURIComponent(n.id)}">
    ...
    `];constructor(){super(),this.heading="",this._arenas=[],this._error="",this._timer=null}connectedCallback(){super.connectedCallback(),this._load(),this._timer=setInterval(()=>this._load(),At)}disconnectedCallback(){this._timer&&(clearInterval(this._timer),this._timer=null),super.disconnectedCallback()}async load(){await this._load()}async _load(){try{let e=await fetch(`${ge}/api/arena`),t=await e.json();if(!e.ok)throw new Error(t.error||`Unable to load Arenas (${e.status})`);let i=Date.now();if(!(t.arenas||[]).some(r=>r.endTime>i&&r.status!=="finished")){let r=Pt(i);if(Math.floor((r-i)/6e4)>=kt){let l=new Date(r),o=String(l.getUTCHours()).padStart(2,"0"),d=String(l.getUTCMinutes()).padStart(2,"0"),h=`${l.getUTCFullYear()}${String(l.getUTCMonth()+1).padStart(2,"0")}${String(l.getUTCDate()).padStart(2,"0")}-${o}${d}`,p=ot[l.getUTCHours()%ot.length];if(await fetch(`${ge}/api/arena`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`arena-hourly-${h}`,creatorId:"hourly-arena",creatorName:`${p.name} ${o}:${d} UTC`,ruleType:p.ruleType,options:p.options,endTime:r})}),e=await fetch(`${ge}/api/arena`),t=await e.json(),!e.ok)throw new Error(t.error||`Unable to reload Arenas (${e.status})`)}}this._arenas=(t.arenas||[]).sort((r,a)=>a.createdAt-r.createdAt),this._error="",this.dispatchEvent(new CustomEvent("arenas-loaded",{detail:{arenas:this._arenas},bubbles:!0,composed:!0}))}catch(e){this._error=e.message||"Unable to load Arenas."}}get _activeArenas(){let e=Date.now();return this._arenas.filter(t=>t.endTime>e&&t.status!=="finished")}render(){let e=this._activeArenas;return c`
        ...
        `}};customElements.define("active-arenas",fe);var Mt=[[4352,4447],[11904,42191],[44032,55203],[63744,64255],[65040,65135],[65280,65376],[65504,65510],[127744,129791],[131072,195103]],lt=n=>{let e=0;for(let t of n){let i=t.codePointAt(0);e+=Mt.some(([s,r])=>i>=s&&i<=r)?2:1}return Math.max(e,1)},xe=class extends Y{static properties={_dotColor:{state:!0}};static styles=Ke;constructor(){super(),this._clientId=m.clientId,this._name=m.userName,this._dotColor=m.isForcedId?"#9fca10ff":"#4caf50"}_commit(e){let t=e.trim().slice(0,12)||"Anonymous";this._name=t,m.set(this._clientId,t),this.dispatchEvent(new CustomEvent("user-name-changed",{bubbles:!0,composed:!0,detail:{userId:this._clientId,userName:t}}))}render(){return G?c``:c`
            ...
            </div>`}};customElements.define("user-badge",xe);var _e=class extends f{static properties={arenaId:{type:String},_messages:{state:!0},_hidden:{state:!0}};static styles=[S,b`
    ...
    `];constructor(){super(),this.standings=[],this.players=[],this.onlineUsers=[],this.expired=!1,this.countdown=""}render(){return c`
            ...
            ${this.standings.length?c`<table class="players">
                    ...
                    <tbody>${this.standings.map(e=>{let t=this.players.find(s=>s.playerId===e.playerId),i=Lt(e.playerId)?!0:this.onlineUsers.some(s=>s.userId===e.playerId);return c`<tr class=${t?.active===!1?"inactive":""}>
                            <td>
