                ...
                <div class="group-body">
                    ${i.map(n=>l`
                        ...
                        <tr><th>Name</th><th></th></tr>
                            ${e[n].slice(0,4).map((a,c)=>l`<tr><td>${Me(c)} ${a.name}</td><td><replay-button url="${Ue(`${U}/api/rank/${a.id}?ruletype=${n}&lod=${m.lod}`,m.clientId,m.userName)}" label="${a.score}"></replay-button></td></tr>`)}
                        </table></div>
                    `)}
                </div>
            </div>
...
<td class="replay-col">
                                    ${n.hasReplay?l`<replay-button prefix="${T(n.locationCountry).emoji}" prefixTitle="${T(n.locationCountry).title}" url="${Ue(`${U}/api/match-replay?id=${n.id}&lod=${m.lod}`,m.clientId,m.userName)}"></replay-button>`:T(n.locationCountry).emoji}
                                </td>
                            </tr>`)}
                        </table></div>
                    </div>
                        ...
                        ${i.map(n=>l`
                            ...
                            <tr><th>Name</th><th>Rating</th><th>W</th><th>L</th></tr>
                                ${[...t[n]].sort((a,c)=>c.rating-a.rating).slice(0,4).map((a,c)=>l`<tr>
                                    <td><a href="${U}/player/${encodeURIComponent(a.name)}?ruleType=${n}">${Me(c)} ${a.name}</a></td>
    ...
    `];constructor(){super(),this._messages=new Map,this._unread=new Map,this._lobbyBound=!1}willUpdate(e){e.has("lobby")&&this.lobby&&!this._lobbyBound&&(this._lobbyBound=!0,this.lobby.onChat(t=>{let s=t.senderId,i=[...this._messages.get(s)??[],t];if(this._messages=new Map(this._messages).set(s,i),s!==this.targetId){let n=(this._unread.get(s)??0)+1;this._unread=new Map(this._unread).set(s,n),ye(this,"unread-changed",{userId:s,count:n})}this.requestUpdate()})),e.has("targetId")&&this.targetId&&this._unread.has(this.targetId)&&(this._unread=new Map(this._unread).set(this.targetId,0),ye(this,"unread-changed",{userId:this.targetId,count:0}))}_send(e){e.preventDefault();let t=this.shadowRoot.querySelector("input"),s=t.value.trim();if(!s||!this.lobby||!this.targetId)return;this.lobby.sendChat(this.targetId,s);let n={messageType:"chat",senderId:this.lobby.currentUser.userId,recipientId:this.targetId,text:s},a=[...this._messages.get(this.targetId)??[],n];this._messages=new Map(this._messages).set(this.targetId,a),t.value="",this.requestUpdate()}updated(e){if(e.has("targetId")){let t=this.shadowRoot.querySelector(".thread");t&&(t.scrollTop=t.scrollHeight);let s=this.shadowRoot.querySelector("input");s&&s.focus()}else if(e.has("_messages")){let t=e.get("_messages");if(this._messages.get(this.targetId)!==t?.get(this.targetId)){let s=this.shadowRoot.querySelector(".thread");s&&(s.scrollTop=s.scrollHeight)}}}render(){if(!this.targetId)return l``;let e=this.lobby?.currentUser?.userId,t=this._messages.get(this.targetId)??[];return l`
            <div class="backdrop" @click=${s=>s.target===s.currentTarget&&ye(this,"close")}>
                <div class="modal" role="dialog" aria-modal="true" aria-label="Chat with ${this.targetName}">
                    ...
                    <button class="cancel" @click=${()=>ye(this,"close")}>Close</button>
                </div>
            </div>`}};customElements.define("message-modal",ze);var ve=(r,e,t)=>r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0})),Ge=class extends f{static properties={challenge:{type:Object},sent:{type:Object}};static styles=[$,vt,xt];render(){return this.challenge?this._incoming(this.challenge):this.sent?this._sent(this.sent):l``}_incoming(e){let t=Object.entries(e.options??{}).filter(([s])=>["raceTo","reds"].includes(s)).map(([s,i])=>`${s}: ${i}`);return l`
            <div class="banner">
                ...
                <div class="row">
                    <button class="btn-accept" aria-label="Accept challenge" @click=${()=>ve(this,"accept")}>Accept</button>
                    <button class="btn-decline" aria-label="Decline challenge" @click=${()=>ve(this,"decline")}>Decline</button>
                </div>
            ...
            <div class="banner ${e.status}">
                ...
                <div class="row">
                    ...
                    ${t?l`<button class="btn-leave" @click=${()=>ve(this,"cancel")}>Cancel</button>`:l`<button aria-label="Dismiss" @click=${()=>ve(this,"dismiss")}>✕</button>`}
                </div>
            </div>`}};customElements.define("challenge-banner",Ge);var as=[{userId:"bot-clawbreak",userName:"ClawBreak",isBot:!0,meta:{country:"BOT"}},{userId:"bot-thefarjaw",userName:"TheFarJaw",isBot:!0,meta:{country:"BOT"}}],A=(r,e,t)=>r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0})),Ye=class extends f{static properties={users:{type:Array},myId:{type:String},myName:{type:String},tableId:{type:String},isChallengePending:{type:Boolean},challenges:{type:Object},pendingChats:{type:Object}};static styles=[$,yt,b`
    ...
    `];render(){let e=(this.users||[]).filter(t=>t.userId!==this.myId);return e.length===0?l`<div class="empty">No other players online yet. Invite a friend!</div>`:l`<ul aria-label="Online players">${jt(e,t=>t.userId,t=>this._row(t))}</ul>`}_row(e){let t=this.pendingChats?.get(e.userId)>0,i=!(this.challenges?.[e.userId]?.challengerId===e.userId)&&(e.isBot||Ht(e,this.myId)),n=!e.isBot&&Be(e)==="playing"&&Bt(e,this.tableId),a=At(e.meta?.origin??"",e.ruleType??"",Be(e)),c=t?l`<button class="btn-chat" aria-label="Unread message from ${e.userName}" @click=${()=>A(this,"open-chat",e.userId)}>💬</button>`:n?l`<button class="btn-spectate" aria-label="Spectate ${e.userName}'s game" @click=${()=>A(this,"spectate",e)}>Spectate</button>`:i?l`<button class="btn-challenge" aria-label="Challenge ${e.userName}" ?disabled=${this.isChallengePending} @click=${()=>E?window.location.href="https://billiards.tailuge.workers.dev/lobby":A(this,"challenge",e.userId)}>Challenge</button>`:l``;return l`
            ...
            </li>`}},Ve=class r extends f{static properties={userId:{type:String},userName:{type:String}};static styles=[$,j,ne];static RULES=[{id:"eightball",label:"Eight Ball",img:"assets/eightball.png"},{id:"nineball",label:"Nine Ball",img:"assets/nineball.png"},{id:"threecushion",label:"Three Cushion (3)",img:"assets/threecushion.png",options:{raceTo:"3"}},{id:"threecushion",label:"Three Cushion (7)",img:"assets/threecushion.png",options:{raceTo:"7"}},{id:"threecushion",label:"Three Cushion (15)",img:"assets/threecushion.png",options:{raceTo:"15"}},{id:"snooker",label:"Snooker (15 reds)",img:"assets/snooker.png"},{id:"snooker",label:"Snooker (6 reds)",img:"assets/snooker.png",options:{reds:"6"}}];render(){return this.userId?l`
            ...
            </div>`:l``}},qe=class extends f{static styles=[$,St];#t={...Tt};#i=null;#e;#r;#d;#s=null;#o=null;#l=null;#p=new Map;#n=null;constructor(){super(),this.#e=m.clientId,this.#r=m.userName;let e=new URLSearchParams(location.search),t=e.get("rematch");t?this.#s=new fe(JSON.parse(decodeURIComponent(t))):this.#s=null;let s=e.get("action");if(s==="join"&&(this.#n={opponentId:e.get("opponentId"),opponentName:e.get("opponentName"),ruleType:e.get("ruletype")}),t||s==="join"){let n=new URL(location);n.searchParams.delete("rematch"),n.searchParams.delete("action"),n.searchParams.delete("opponentId"),n.searchParams.delete("opponentName"),n.searchParams.delete("ruletype"),history.replaceState(null,"",n)}let i="https://billiards-network.onrender.com";(location.hostname==="localhost"||location.hostname==="127.0.0.1")&&(i=`${location.protocol==="https:"?"https:":"http:"}//${location.host}`),this.#d=new be({baseUrl:i}),this.#d.setVersion(oe(ae))}connectedCallback(){super.connectedCallback(),this._onUserChanged=e=>{this.#e=e.detail.userId,this.#r=e.detail.userName,this.#i?this.#i.updatePresence({userId:this.#e,userName:this.#r}).catch(t=>console.error("Failed to update presence:",t)):this._connect().catch(t=>console.error("Lobby connect failed:",t))},document.addEventListener("user-name-changed",this._onUserChanged),this._connect().catch(e=>console.error("Lobby connect failed:",e))}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("user-name-changed",this._onUserChanged),this.#i?.leave()}dispatch(e){this.#t=Ct(this.#t,{...e,myId:this.#e}),this.requestUpdate()}get state(){return this.#t}get#g(){return this.#t.connected}get#b(){return this.#t.users}get#u(){return this.#t.currentMatch?.tableId}get#f(){return this.#t.currentMatch?.ruleType||"standard"}get#y(){return!!this.#t.currentMatch?.isFirst}get#v(){return this.#t.currentMatch?.options}get#m(){return Object.values(this.#t.challenges).find(e=>e.challengeeId===this.#e&&e.status==="pending")}get#c(){return Object.values(this.#t.challenges).find(e=>e.challengerId===this.#e)}get#a(){return[...this.#b,...as]}async _connect(){if(this.#i=await this.#d.joinLobby({messageType:"presence",type:"join",userId:this.#e,userName:this.#r}),this.dispatch({type:"CONNECTED",payload:!0}),this.#s&&this.#s.shouldChallenge(this.#e)){let e=this.#s.info,t=await this.#s.sendChallenge(this.#i);this.dispatch({type:"CHALLENGE_SENT",payload:{challengerId:this.#e,challengeeId:this.#s.opponentId,recipientName:e.opponentName,ruleType:this.#s.ruleType,options:e.options,tableId:t,rematch:e}}),this.#s=null}this.#i.onUsersChange(e=>this.dispatch({type:"USERS_UPDATE",payload:e})),this.#i.onChallenge(e=>{if(e.type==="offer"&&e.challengeeId===this.#e){if(this.#s?.shouldAutoAccept(e)){this.dispatch({type:"CHALLENGE_MSG",payload:e}),this.#h().catch(s=>console.error("Auto-accept failed:",s));return}if(this.#n&&this.#n.opponentId===e.challengerId){this.#n=null,this.dispatch({type:"CHALLENGE_MSG",payload:e}),this.#h(e.challengerId).catch(s=>{console.error("Auto-join accept failed:",s)});return}let t=this.#c;if(t&&t.challengeeId===e.challengerId&&t.status==="pending"){this.#e<e.challengerId&&(this.dispatch({type:"CHALLENGE_MSG",payload:e}),this.#h().catch(s=>console.error("Simultaneous auto-accept failed:",s)));return}}this.dispatch({type:"CHALLENGE_MSG",payload:e}),e.type==="offer"&&e.challengeeId===this.#e&&document.hidden&&Notification.permission==="granted"&&new Notification("Challenge received!",{body:`${e.challengerName} challenged you to ${e.ruleType}`,icon:"assets/threecushion.png"})})}async#x(e,t,s){this.#n=null;let i=this.#a.find(a=>a.userId===e);if(i?.isBot){let a="bot-"+Math.random().toString(36).slice(2,8),c=le(this.#e,this.#e,this.#s?.info);window.location.href=Ne({tableId:a,userId:this.#e,userName:this.#r,ruleType:t,isFirst:c,options:s,bot:i.userName,lod:m.lod,flip:m.flip,rematch:this.#s?.rematchParam});return}let n=await this.#i.challenge(e,t,void 0,s);De("createTable"),this.dispatch({type:"CHALLENGE_SENT",payload:{challengerId:this.#e,challengeeId:e,recipientName:i?.userName||e,ruleType:t,options:s,tableId:n}})}async#$(){let e=this.#c;e?.status==="pending"&&(await this.#i.cancelChallenge(e.challengeeId,e.ruleType),this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengeeId}))}async#h(e){this.#n=null;let t=e?this.#t.challenges[e]:this.#m;if(!t)return;await this.#i.acceptChallenge(t.challengerId,t.ruleType,t.tableId,t.options,t.challengerName),De("joinTable");let s=this.#s?.rematchParam??(t.rematch?encodeURIComponent(JSON.stringify(t.rematch)):void 0);this.dispatch({type:"CHALLENGE_DISMISS",payload:t.challengerId}),this.dispatch({type:"MATCH_SET",payload:{tableId:t.tableId,ruleType:t.ruleType,options:t.options,isFirst:le(this.#e,t.challengerId,t.rematch),rematch:s}})}async#w(){this.#n=null;let e=this.#m;await this.#i.declineChallenge(e.challengerId,e.ruleType,e.challengerName),this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengerId})}#_(){let e=this.#c;e&&this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengeeId})}#S(){let e=[...this.#a].filter(t=>t.meta?.country!=="BOT").map(t=>{let{meta:s={},...i}=t,{ts:n,since:a,...c}=s;return{...i,meta:c}});console.log(JSON.stringify(e,null,2)),console.log(JSON.stringify({myId:this.#e,myName:this.#r}))}render(){if(this.#u){let t=Ne({tableId:this.#u,userId:this.#e,userName:this.#r,ruleType:this.#f,isFirst:this.#y,options:this.#v,lod:m.lod,flip:m.flip,rematch:this.#t.currentMatch?.rematch});return this.#t={...this.#t,currentMatch:null},window.location.href=t,l``}let e=this.#o;return l`
            ...
            </message-modal>`}};customElements.define("user-list",Ye);customElements.define("challenge-modal",Ve);customElements.define("online-panel",qe);var Fe=class extends w{static properties={_dotColor:{state:!0}};static styles=$t;constructor(){super(),this._clientId=m.clientId,this._name=m.userName,this._dotColor=m.isForcedId?"#9fca10ff":"#4caf50"}_commit(e){let t=e.trim().slice(0,12)||"Anonymous";this._name=t,m.set(this._clientId,t),this.dispatchEvent(new CustomEvent("user-name-changed",{bubbles:!0,composed:!0,detail:{userId:this._clientId,userName:t}}))}render(){return E?l``:l`
    ...
    `];constructor(){super(),this._open=!1,this._showStats=!1,this._copied=!1,this._theme=document.documentElement.getAttribute("theme")||"light",this._notifEnabled=Notification.permission==="granted",this._onKeydown=this._onKeydown.bind(this)}connectedCallback(){super.connectedCallback(),window.addEventListener("keydown",this._onKeydown)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("keydown",this._onKeydown)}_onKeydown(e){this._open&&e.key==="Escape"&&this._close()}_toggle(e){e.stopPropagation(),this._open=!this._open}_close(){this._open=!1}_setTheme(e){let t=e.target.checked?"dark":"light";this._theme=t,document.documentElement.setAttribute("theme",t),localStorage.setItem("theme",t),this.dispatchEvent(new CustomEvent("theme-changed",{detail:t,bubbles:!0,composed:!0}))}_share(){navigator.share&&/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)?navigator.share({title:document.title,url:location.href}):navigator.clipboard.writeText(location.href).then(()=>{this._copied=!0,setTimeout(()=>{this._copied=!1},2e3)})}async _toggleNotifications(e){if(e.target.checked){let t=await Notification.requestPermission();this._notifEnabled=t==="granted"}else this._notifEnabled=!1;this.requestUpdate()}render(){return l`
        ...
        `}};customElements.define("settings-modal",We);var W=class extends H{constructor(e){if(super(e),this.it=y,e.type!==ce.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(e){if(e===y||e==null)return this._t=void 0,this.it=e;if(e===x)return e;if(typeof e!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(e===this.it)return this._t;this.it=e;let t=[e];return t.raw=t,this._t={_$litType$:this.constructor.resultType,strings:t,values:[]}}};W.directiveName="unsafeHTML",W.resultType=1;var zt=he(W);var Gt=['This game is free to play and open source on <a href="https://github.com/tailuge/billiards" target="_blank">GitHub</a>',"Choose graphics settings in options menu.",'Masse trick shot replay: <a href="https://scoreboard-tailuge.vercel.app/api/replay/534?lod=4">here</a>.',"You can change your name by clicking on the user badge.",'Do you know <a href="https://www.youtube.com/watch?v=ArNBvY1uEUo" target="_blank">Three Cushion</a> billiards rules?',"Thank you for playing.",'A great three cushion <a href="https://scoreboard-tailuge.vercel.app/api/rank/24872636?ruletype=threecushion&lod=4">break</a>'],Ke=class extends f{static styles=b`
        :host {
            display: block;
            text-align: center;
            font-size: 0.75rem;
    ...
    `;constructor(){super(),this.msg=Gt[Math.floor(Math.random()*Gt.length)]}render(){return l`${zt(this.msg)}`}};customElements.define("motd-panel",Ke);var Xe=class extends f{static properties={_theme:{type:String,reflect:!0,attribute:"theme"}};static styles=It;constructor(){super(),console.log("URL:",window.location.href),console.log("Search params:",Object.fromEntries(new URLSearchParams(window.location.search))),this._theme=document.documentElement.getAttribute("theme")||"light"}get _ctrl(){return this.shadowRoot.querySelector("online-panel")}render(){return l`
            <div class="container">
                <header class="topbar">
