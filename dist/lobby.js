                ...
                <div class="group-body">
                    ${i.map(n=>o`
                        ...
                        <tr><th>Name</th><th></th></tr>
                            ${e[n].slice(0,4).map((a,c)=>o`<tr><td>${Me(c)} ${a.name}</td><td><replay-button url="${Ue(`${R}/api/rank/${a.id}?ruletype=${n}&lod=${u.lod}`,u.clientId,u.userName)}" label="${a.score}"></replay-button></td></tr>`)}
                        </table></div>
                    `)}
                </div>
            </div>
...
<td class="replay-col">
                                    ${n.hasReplay?o`<replay-button prefix="${T(n.locationCountry).emoji}" prefixTitle="${T(n.locationCountry).title}" url="${Ue(`${R}/api/match-replay?id=${n.id}&lod=${u.lod}`,u.clientId,u.userName)}"></replay-button>`:T(n.locationCountry).emoji}
                                </td>
                            </tr>`)}
                        </table></div>
                    </div>
    ...
    `];constructor(){super(),this._messages=new Map,this._unread=new Map,this._lobbyBound=!1}willUpdate(e){e.has("lobby")&&this.lobby&&!this._lobbyBound&&(this._lobbyBound=!0,this.lobby.onChat(t=>{let s=t.senderId,i=[...this._messages.get(s)??[],t];if(this._messages=new Map(this._messages).set(s,i),s!==this.targetId){let n=(this._unread.get(s)??0)+1;this._unread=new Map(this._unread).set(s,n),ve(this,"unread-changed",{userId:s,count:n})}this.requestUpdate()})),e.has("targetId")&&this.targetId&&this._unread.has(this.targetId)&&(this._unread=new Map(this._unread).set(this.targetId,0),ve(this,"unread-changed",{userId:this.targetId,count:0}))}_send(e){e.preventDefault();let t=this.shadowRoot.querySelector("input"),s=t.value.trim();if(!s||!this.lobby||!this.targetId)return;this.lobby.sendChat(this.targetId,s);let n={messageType:"chat",senderId:this.lobby.currentUser.userId,recipientId:this.targetId,text:s},a=[...this._messages.get(this.targetId)??[],n];this._messages=new Map(this._messages).set(this.targetId,a),t.value="",this.requestUpdate()}updated(e){if(e.has("targetId")){let t=this.shadowRoot.querySelector(".thread");t&&(t.scrollTop=t.scrollHeight);let s=this.shadowRoot.querySelector("input");s&&s.focus()}else if(e.has("_messages")){let t=e.get("_messages");if(this._messages.get(this.targetId)!==t?.get(this.targetId)){let s=this.shadowRoot.querySelector(".thread");s&&(s.scrollTop=s.scrollHeight)}}}render(){if(!this.targetId)return o``;let e=this.lobby?.currentUser?.userId,t=this._messages.get(this.targetId)??[];return o`
            ...
            </div>`}};customElements.define("message-modal",ze);var xe=(r,e,t)=>r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0})),Ye=class extends f{static properties={challenge:{type:Object},sent:{type:Object}};static styles=[_,yt,vt];render(){return this.challenge?this._incoming(this.challenge):this.sent?this._sent(this.sent):o``}_incoming(e){let t=Object.entries(e.options??{}).filter(([s])=>["raceTo","reds"].includes(s)).map(([s,i])=>`${s}: ${i}`);return o`
            <div class="banner">
            ...
            <div class="banner ${e.status}">
                ...
                <div class="row">
                    ...
                    ${t?o`<button class="btn-leave" @click=${()=>xe(this,"cancel")}>Cancel</button>`:o`<button aria-label="Dismiss" @click=${()=>xe(this,"dismiss")}>✕</button>`}
                </div>
    ...
    `];render(){let e=(this.users||[]).filter(t=>t.userId!==this.myId);return e.length===0?o`<div class="empty">No other players online yet. Invite a friend!</div>`:o`<ul aria-label="Online players">${Ot(e,t=>t.userId,t=>this._row(t))}</ul>`}_row(e){let t=this.pendingChats?.get(e.userId)>0,i=!(this.challenges?.[e.userId]?.challengerId===e.userId)&&(e.isBot||Ht(e,this.myId)),n=!e.isBot&&Be(e)==="playing"&&jt(e,this.tableId),a=Tt(e.meta?.origin??"",e.ruleType??"",Be(e)),c=t?o`<button class="btn-chat" aria-label="Unread message from ${e.userName}" @click=${()=>L(this,"open-chat",e.userId)}>💬</button>`:n?o`<button class="btn-spectate" aria-label="Spectate ${e.userName}'s game" @click=${()=>L(this,"spectate",e)}>Spectate</button>`:i?o`<button class="btn-challenge" aria-label="Challenge ${e.userName}" ?disabled=${this.isChallengePending} @click=${()=>C?window.location.href="https://billiards.tailuge.workers.dev/lobby":L(this,"challenge",e.userId)}>Challenge</button>`:o``;return o`
    ...
    `;constructor(){super(),this.msg=zt[Math.floor(Math.random()*zt.length)]}render(){return o`${Dt(this.msg)}`}};customElements.define("motd-panel",Ke);var Xe=class extends f{static properties={_theme:{type:String,reflect:!0,attribute:"theme"}};static styles=St;constructor(){super(),console.log("URL:",window.location.href),console.log("Search params:",Object.fromEntries(new URLSearchParams(window.location.search))),this._theme=document.documentElement.getAttribute("theme")||"light"}get _ctrl(){return this.shadowRoot.querySelector("online-panel")}render(){return o`
            <div class="container">
                <header class="topbar">
