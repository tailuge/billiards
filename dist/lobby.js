...
<td class="replay-col">
    ...
    `];render(){let e=this.slots||[];if(e.filter(i=>i.status==="online").length===0)return c`<div class="empty">No other players online yet. Invite a friend!</div>`;let s=new Set(Ht(this.users||[]).filter(i=>i.players.length>1).map(i=>i.tableId));return c`<ul aria-label="Online players">${qt(e,(i,n)=>n,(i,n)=>this._rowSlot(i,n,s))}</ul>`}_rowSlot(e,t,s){if(e.status==="offline"){let i=e.user,n=Re(i.meta?.origin??"",i.ruleType??"",pe(i));return c`
    ...
    `];constructor(){super(),this._messages=new Map,this._unread=new Map,this._lobbyBound=!1}willUpdate(e){e.has("lobby")&&this.lobby&&!this._lobbyBound&&(this._lobbyBound=!0,this.lobby.onChat(t=>{let s=t.senderId,i=[...this._messages.get(s)??[],t];if(this._messages=new Map(this._messages).set(s,i),s!==this.targetId){let n=(this._unread.get(s)??0)+1;this._unread=new Map(this._unread).set(s,n),$e(this,"unread-changed",{userId:s,count:n})}this.requestUpdate()})),e.has("targetId")&&this.targetId&&this._unread.has(this.targetId)&&(this._unread=new Map(this._unread).set(this.targetId,0),$e(this,"unread-changed",{userId:this.targetId,count:0}))}_send(e){e.preventDefault();let t=this.shadowRoot.querySelector("input"),s=t.value.trim();if(!s||!this.lobby||!this.targetId)return;this.lobby.sendChat(this.targetId,s);let n={messageType:"chat",senderId:this.lobby.currentUser.userId,recipientId:this.targetId,text:s},o=[...this._messages.get(this.targetId)??[],n];this._messages=new Map(this._messages).set(this.targetId,o),t.value="",this.requestUpdate()}updated(e){if(e.has("targetId")){let t=this.shadowRoot.querySelector(".thread");t&&(t.scrollTop=t.scrollHeight);let s=this.shadowRoot.querySelector("input");s&&s.focus()}else if(e.has("_messages")){let t=e.get("_messages");if(this._messages.get(this.targetId)!==t?.get(this.targetId)){let s=this.shadowRoot.querySelector(".thread");s&&(s.scrollTop=s.scrollHeight)}}}render(){if(!this.targetId)return c``;let e=this.lobby?.currentUser?.userId,t=this._messages.get(this.targetId)??[];return c`
            ...
            </div>`:c``}},Xe=class extends b{static styles=[S,Ct];#e={...kt,settled:!1};#i=null;#t;#r;#c;#o=null;#a=null;#d=new Map;#m=new fe;#s=null;#h=!1;constructor(){super(),this.#t=u.clientId,this.#r=u.userName;let e=new URLSearchParams(location.search),t=e.get("opponentId");if(t){this.#s={opponentId:t,opponentName:e.get("opponentName")||t,ruleType:e.get("ruletype")||"nineball",nextTurnId:e.get("nextTurnId")};let i=new URL(location.href);i.searchParams.delete("opponentId"),i.searchParams.delete("opponentName"),i.searchParams.delete("ruletype"),i.searchParams.delete("nextTurnId"),history.replaceState(null,"",i)}let s=`https://${K}`;(location.hostname==="localhost"||location.hostname==="127.0.0.1")&&(s=`${location.protocol==="https:"?"https:":"http:"}//${location.host}`),this.#c=new me({baseUrl:s}),this.#c.setVersion(he(ce))}connectedCallback(){super.connectedCallback(),this._onUserChanged=e=>{this.#t=e.detail.userId,this.#r=e.detail.userName,this.#i?this.#i.updatePresence({userId:this.#t,userName:this.#r}).catch(t=>console.error("Failed to update presence:",t)):this._connect().catch(t=>console.error("Lobby connect failed:",t))},document.addEventListener("user-name-changed",this._onUserChanged),this._connect().catch(e=>console.error("Lobby connect failed:",e))}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("user-name-changed",this._onUserChanged),this.#i?.leave()}dispatch(e){this.#e=Pt(this.#e,{...e,myId:this.#t}),e.type==="CHALLENGE_MSG"&&this.#$(e.payload),this.requestUpdate()}#x(){if(!this.#s||!this.#e.connected)return;let e=this.#s.opponentId;!this.#n&&this.#e.users.some(t=>t.userId===e)&&this.#v(e,this.#s.ruleType,this.#s.options,this.#s.nextTurnId)}#$(e){if(this.#s&&(e.challengerId===this.#s.opponentId||e.challengeeId===this.#s.opponentId)&&(e.type==="decline"||e.type==="cancel")&&(this.#s=null),e.type==="offer"&&e.challengeeId===this.#t&&this.#s&&this.#s.opponentId===e.challengerId){let t=this.#s.nextTurnId;this.#s=null;let s=this.#n;s&&s.challengeeId===e.challengerId&&s.status==="pending"?this.#t<e.challengerId&&this.#g(e.challengerId,t).catch(i=>console.error("Simultaneous auto-accept failed:",i)):this.#g(e.challengerId,t).catch(i=>console.error("Auto-join accept failed:",i))}}get state(){return this.#e}get#f(){return this.#e.connected}get#b(){return this.#e.users}get#p(){return this.#e.currentMatch?.tableId}get#w(){return this.#e.currentMatch?.ruleType||"standard"}get#S(){return!!this.#e.currentMatch?.isFirst}get#_(){return this.#e.currentMatch?.options}get#u(){return Object.values(this.#e.challenges).find(e=>e.challengeeId===this.#t&&e.status==="pending")}get#n(){return Object.values(this.#e.challenges).find(e=>e.challengerId===this.#t)}get#l(){return[...this.#b,...Wt]}get#y(){return this.#m.getSlots()}async _connect(){this.#i=await this.#c.joinLobby({messageType:"presence",type:"join",userId:this.#t,userName:this.#r}),this.dispatch({type:"CONNECTED",payload:!0}),this.#h=!1,this.dispatch({type:"SETTLED",payload:!1}),this.#i.onUsersChange(e=>{let t=[...e,...Wt].filter(s=>s.userId!==this.#t);this.#m.update(t),this.dispatch({type:"USERS_UPDATE",payload:e})}),this.#i.onChallenge(e=>{this.dispatch({type:"CHALLENGE_MSG",payload:e}),this.#i.settled&&e.type==="offer"&&e.challengeeId===this.#t&&document.hidden&&Notification.permission==="granted"&&new Notification("Challenge received!",{body:`${e.challengerName} challenged you to ${e.ruleType}`,icon:"assets/threecushion.png"})}),this.#i.onSettled(()=>{this.#h=!0,this.dispatch({type:"SETTLED",payload:!0}),this.#x()})}async#v(e,t,s,i){this.#s&&this.#s.opponentId===e||(this.#s=null);let o=this.#l.find(a=>a.userId===e);if(o?.isBot){let a="bot-"+Math.random().toString(36).slice(2,8),p=!0;window.location.href=je({tableId:a,userId:this.#t,userName:this.#r,ruleType:t,isFirst:p,options:s,bot:o.userName,lod:u.lod,flip:u.flip});return}let l=this.#i?await this.#i.challenge(e,t,s,i):"test-"+Math.random().toString(36).slice(2,7);qe("createTable"),this.dispatch({type:"CHALLENGE_SENT",payload:{challengerId:this.#t,challengeeId:e,recipientName:o?.userName||e,ruleType:t,options:s,tableId:l,nextTurnId:i}})}async#I(){this.#s=null;let e=this.#n;e?.status==="pending"&&(this.#i&&await this.#i.cancelChallenge(e.challengeeId,e.ruleType),this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengeeId}))}async#g(e,t){let s=e?this.#e.challenges[e]:this.#u;s&&(this.#i&&await this.#i.acceptChallenge(s.challengerId,s.ruleType,s.tableId,s.options,s.challengerName,t),qe("joinTable"),this.dispatch({type:"CHALLENGE_MSG",payload:{type:"accept",challengerId:s.challengerId,challengerName:s.challengerName,challengeeId:this.#t,ruleType:s.ruleType,tableId:s.tableId,options:s.options,nextTurnId:t}}),this.#s=null)}async#E(){this.#s=null;let e=this.#u;this.#i&&await this.#i.declineChallenge(e.challengerId,e.ruleType,e.challengerName),this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengerId})}#T(){this.#s=null;let e=this.#n;e&&this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengeeId})}#C(){let e=[...this.#l].filter(t=>t.meta?.country!=="BOT").map(t=>{let{meta:s={},...i}=t,{ts:n,since:o,...l}=s;return{...i,meta:l}});console.log("=== USERS ==="),console.log(JSON.stringify(e,null,2)),console.log("=== MY INFO ==="),console.log(JSON.stringify({myId:this.#t,myName:this.#r})),console.log("=== NCHAN RECORDED MESSAGES ==="),console.log(this.#c.recordedMessages),console.log("=== SLOTS ==="),console.table(this.#y.map(t=>({userId:t.userId,status:t.status,offlineSince:t.offlineSince,online:t.status==="online"?"\u2713":"\u2717"})))}render(){if(this.#p){let t=je({tableId:this.#p,userId:this.#t,userName:this.#r,ruleType:this.#w,isFirst:this.#S,options:this.#_,lod:u.lod,flip:u.flip});return this.#s=null,this.#e={...this.#e,currentMatch:null},window.location.href=t,c``}let e=this.#o;return c`
