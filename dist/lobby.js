...
<td class="replay-col">
    ...
    `];updated(){if(!this.#e)return;let e=this.renderRoot.querySelector("ul");if(!e)return;let t=(this.slots||[]).filter(s=>s.status==="online").length;e.style.setProperty("--ul-expanded-height",t*36+8+"px")}#s(){this.#e=!this.#e,Q(this,"user-list-toggle",{expanded:this.#e}),this.requestUpdate()}render(){let e=this.slots||[];if(e.filter(i=>i.status==="online").length===0)return c`<div class="empty">No other players online yet. Invite a friend!</div>`;let s=new Set(Ht(this.users||[]).filter(i=>i.players.length>1).map(i=>i.tableId));return c`
