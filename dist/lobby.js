                ...
                <div class="group-body">
                    ${i.map(n=>l`
                        ...
                        <tr><th>Name</th><th></th></tr>
                            ${e[n].slice(0,4).map((a,c)=>l`<tr><td>${Me(c)} ${a.name}</td><td><replay-button url="${Ue(`${U}/api/rank/${a.id}?ruletype=${n}&lod=${g.lod}`,g.clientId,g.userName)}" label="${a.score}"></replay-button></td></tr>`)}
                        </table></div>
                    `)}
                </div>
            </div>
...
<td class="replay-col">
                                    ${n.hasReplay?l`<replay-button prefix="${T(n.locationCountry).emoji}" prefixTitle="${T(n.locationCountry).title}" url="${Ue(`${U}/api/match-replay?id=${n.id}&lod=${g.lod}`,g.clientId,g.userName)}"></replay-button>`:T(n.locationCountry).emoji}
                                </td>
                            </tr>`)}
                        </table></div>
                    </div>
    ...
    `];constructor(){super(),this._messages=new Map,this._unread=new Map,this._lobbyBound=!1}willUpdate(e){e.has("lobby")&&this.lobby&&!this._lobbyBound&&(this._lobbyBound=!0,this.lobby.onChat(t=>{let s=t.senderId,i=[...this._messages.get(s)??[],t];if(this._messages=new Map(this._messages).set(s,i),s!==this.targetId){let n=(this._unread.get(s)??0)+1;this._unread=new Map(this._unread).set(s,n),ye(this,"unread-changed",{userId:s,count:n})}this.requestUpdate()})),e.has("targetId")&&this.targetId&&this._unread.has(this.targetId)&&(this._unread=new Map(this._unread).set(this.targetId,0),ye(this,"unread-changed",{userId:this.targetId,count:0}))}_send(e){e.preventDefault();let t=this.shadowRoot.querySelector("input"),s=t.value.trim();if(!s||!this.lobby||!this.targetId)return;this.lobby.sendChat(this.targetId,s);let n={messageType:"chat",senderId:this.lobby.currentUser.userId,recipientId:this.targetId,text:s},a=[...this._messages.get(this.targetId)??[],n];this._messages=new Map(this._messages).set(this.targetId,a),t.value="",this.requestUpdate()}updated(e){if(e.has("targetId")){let t=this.shadowRoot.querySelector(".thread");t&&(t.scrollTop=t.scrollHeight);let s=this.shadowRoot.querySelector("input");s&&s.focus()}else if(e.has("_messages")){let t=e.get("_messages");if(this._messages.get(this.targetId)!==t?.get(this.targetId)){let s=this.shadowRoot.querySelector(".thread");s&&(s.scrollTop=s.scrollHeight)}}}render(){if(!this.targetId)return l``;let e=this.lobby?.currentUser?.userId,t=this._messages.get(this.targetId)??[];return l`
            ...
            </div>`}};customElements.define("message-modal",ze);var ve=(r,e,t)=>r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0})),Ye=class extends f{static properties={challenge:{type:Object},sent:{type:Object}};static styles=[$,vt,xt];render(){return this.challenge?this._incoming(this.challenge):this.sent?this._sent(this.sent):l``}_incoming(e){let t=Object.entries(e.options??{}).filter(([s])=>["raceTo","reds"].includes(s)).map(([s,i])=>`${s}: ${i}`);return l`
            <div class="banner">
            ...
            </div>`}};customElements.define("challenge-banner",Ye);var as=[{userId:"bot-clawbreak",userName:"ClawBreak",isBot:!0,meta:{country:"BOT"}},{userId:"bot-thefarjaw",userName:"TheFarJaw",isBot:!0,meta:{country:"BOT"}}],A=(r,e,t)=>r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0})),Ge=class extends f{static properties={users:{type:Array},myId:{type:String},myName:{type:String},tableId:{type:String},isChallengePending:{type:Boolean},challenges:{type:Object},pendingChats:{type:Object}};static styles=[$,yt,b`
    ...
    `];render(){let e=(this.users||[]).filter(t=>t.userId!==this.myId);return e.length===0?l`<div class="empty">No other players online yet. Invite a friend!</div>`:l`<ul aria-label="Online players">${Ht(e,t=>t.userId,t=>this._row(t))}</ul>`}_row(e){let t=this.pendingChats?.get(e.userId)>0,i=!(this.challenges?.[e.userId]?.challengerId===e.userId)&&(e.isBot||jt(e,this.myId)),n=!e.isBot&&Be(e)==="playing"&&Bt(e,this.tableId),a=At(e.meta?.origin??"",e.ruleType??"",Be(e)),c=t?l`<button class="btn-chat" aria-label="Unread message from ${e.userName}" @click=${()=>A(this,"open-chat",e.userId)}>💬</button>`:n?l`<button class="btn-spectate" aria-label="Spectate ${e.userName}'s game" @click=${()=>A(this,"spectate",e)}>Spectate</button>`:i?l`<button class="btn-challenge" aria-label="Challenge ${e.userName}" ?disabled=${this.isChallengePending} @click=${()=>E?window.location.href="https://billiards.tailuge.workers.dev/lobby":A(this,"challenge",e.userId)}>Challenge</button>`:l``;return l`
            ...
            </li>`}},Ve=class r extends f{static properties={userId:{type:String},userName:{type:String}};static styles=[$,H,ne];static RULES=[{id:"eightball",label:"Eight Ball",img:"assets/eightball.png"},{id:"nineball",label:"Nine Ball",img:"assets/nineball.png"},{id:"threecushion",label:"Three Cushion (3)",img:"assets/threecushion.png",options:{raceTo:"3"}},{id:"threecushion",label:"Three Cushion (7)",img:"assets/threecushion.png",options:{raceTo:"7"}},{id:"threecushion",label:"Three Cushion (15)",img:"assets/threecushion.png",options:{raceTo:"15"}},{id:"snooker",label:"Snooker (15 reds)",img:"assets/snooker.png"},{id:"snooker",label:"Snooker (6 reds)",img:"assets/snooker.png",options:{reds:"6"}}];render(){return this.userId?l`
            ...
            </div>`:l``}},qe=class extends f{static styles=[$,St];#t={...Tt};#i=null;#e;#r;#c;#s=null;#a=null;#o=null;#h=new Map;constructor(){super(),this.#e=g.clientId,this.#r=g.userName;let t=new URLSearchParams(location.search).get("rematch");if(t){this.#s=new fe(JSON.parse(decodeURIComponent(t)));let i=new URL(location);i.searchParams.delete("rematch"),history.replaceState(null,"",i)}else this.#s=null;let s="https://billiards-network.onrender.com";(location.hostname==="localhost"||location.hostname==="127.0.0.1")&&(s=`${location.protocol==="https:"?"https:":"http:"}//${location.host}`),this.#c=new be({baseUrl:s}),this.#c.setVersion(oe(ae))}connectedCallback(){super.connectedCallback(),this._onUserChanged=e=>{this.#e=e.detail.userId,this.#r=e.detail.userName,this.#i?this.#i.updatePresence({userId:this.#e,userName:this.#r}).catch(t=>console.error("Failed to update presence:",t)):this._connect().catch(t=>console.error("Lobby connect failed:",t))},document.addEventListener("user-name-changed",this._onUserChanged),this._connect().catch(e=>console.error("Lobby connect failed:",e))}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("user-name-changed",this._onUserChanged),this.#i?.leave()}dispatch(e){this.#t=Ct(this.#t,{...e,myId:this.#e}),this.requestUpdate()}get state(){return this.#t}get#g(){return this.#t.connected}get#m(){return this.#t.users}get#d(){return this.#t.currentMatch?.tableId}get#b(){return this.#t.currentMatch?.ruleType||"standard"}get#f(){return!!this.#t.currentMatch?.isFirst}get#y(){return this.#t.currentMatch?.options}get#p(){return Object.values(this.#t.challenges).find(e=>e.challengeeId===this.#e&&e.status==="pending")}get#l(){return Object.values(this.#t.challenges).find(e=>e.challengerId===this.#e)}get#n(){return[...this.#m,...as]}async _connect(){if(this.#i=await this.#c.joinLobby({messageType:"presence",type:"join",userId:this.#e,userName:this.#r}),this.dispatch({type:"CONNECTED",payload:!0}),this.#s&&this.#s.shouldChallenge(this.#e)){let e=this.#s.info,t=await this.#s.sendChallenge(this.#i);this.dispatch({type:"CHALLENGE_SENT",payload:{challengerId:this.#e,challengeeId:this.#s.opponentId,recipientName:e.opponentName,ruleType:this.#s.ruleType,options:e.options,tableId:t,rematch:e}}),this.#s=null}this.#i.onUsersChange(e=>this.dispatch({type:"USERS_UPDATE",payload:e})),this.#i.onChallenge(e=>{if(e.type==="offer"&&e.challengeeId===this.#e){if(this.#s?.shouldAutoAccept(e)){this.dispatch({type:"CHALLENGE_MSG",payload:e}),this.#u().catch(s=>console.error("Auto-accept failed:",s));return}let t=this.#l;if(t&&t.challengeeId===e.challengerId&&t.status==="pending"){this.#e<e.challengerId&&(this.dispatch({type:"CHALLENGE_MSG",payload:e}),this.#u().catch(s=>console.error("Simultaneous auto-accept failed:",s)));return}}this.dispatch({type:"CHALLENGE_MSG",payload:e}),e.type==="offer"&&e.challengeeId===this.#e&&document.hidden&&Notification.permission==="granted"&&new Notification("Challenge received!",{body:`${e.challengerName} challenged you to ${e.ruleType}`,icon:"assets/threecushion.png"})})}async#v(e,t,s){let i=this.#n.find(a=>a.userId===e);if(i?.isBot){let a="bot-"+Math.random().toString(36).slice(2,8),c=le(this.#e,this.#e,this.#s?.info);window.location.href=Ne({tableId:a,userId:this.#e,userName:this.#r,ruleType:t,isFirst:c,options:s,bot:i.userName,lod:g.lod,flip:g.flip,rematch:this.#s?.rematchParam});return}let n=await this.#i.challenge(e,t,void 0,s);De("createTable"),this.dispatch({type:"CHALLENGE_SENT",payload:{challengerId:this.#e,challengeeId:e,recipientName:i?.userName||e,ruleType:t,options:s,tableId:n}})}async#x(){let e=this.#l;e?.status==="pending"&&(await this.#i.cancelChallenge(e.challengeeId,e.ruleType),this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengeeId}))}async#u(){let e=this.#p;await this.#i.acceptChallenge(e.challengerId,e.ruleType,e.tableId,e.options,e.challengerName),De("joinTable");let t=this.#s?.rematchParam??(e.rematch?encodeURIComponent(JSON.stringify(e.rematch)):void 0);this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengerId}),this.dispatch({type:"MATCH_SET",payload:{tableId:e.tableId,ruleType:e.ruleType,options:e.options,isFirst:le(this.#e,e.challengerId,e.rematch),rematch:t}})}async#$(){let e=this.#p;await this.#i.declineChallenge(e.challengerId,e.ruleType,e.challengerName),this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengerId})}#w(){let e=this.#l;e&&this.dispatch({type:"CHALLENGE_DISMISS",payload:e.challengeeId})}#_(){let e=[...this.#n].filter(t=>t.meta?.country!=="BOT").map(t=>{let{meta:s={},...i}=t,{ts:n,since:a,...c}=s;return{...i,meta:c}});console.log(JSON.stringify(e,null,2)),console.log(JSON.stringify({myId:this.#e,myName:this.#r}))}render(){if(this.#d){let t=Ne({tableId:this.#d,userId:this.#e,userName:this.#r,ruleType:this.#b,isFirst:this.#f,options:this.#y,lod:g.lod,flip:g.flip,rematch:this.#t.currentMatch?.rematch});return this.#t={...this.#t,currentMatch:null},window.location.href=t,l``}let e=this.#a;return l`
