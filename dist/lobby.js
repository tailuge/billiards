...
<td class="replay-col">
    ...
    `];updated(){if(!this.#e)return;let e=this.renderRoot.querySelector("ul");if(!e)return;let t=e.scrollHeight;t>0&&e.style.setProperty("--ul-expanded-height",t+"px")}#s(){this.#e=!this.#e,Q(this,"user-list-toggle",{expanded:this.#e}),this.requestUpdate()}render(){let e=this.slots||[];if(e.filter(i=>i.status==="online").length===0)return c`<div class="empty">No other players online yet. Invite a friend!</div>`;let s=new Set(Ht(this.users||[]).filter(i=>i.players.length>1).map(i=>i.tableId));return c`
            ...
            <div class="expand-toggle" @click=${this.#s}>
                ...
                </li>`}return this._row(e.user,s)}_row(e,t){let s=this.pendingChats?.get(e.userId)>0,n=!(this.challenges?.[e.userId]?.challengerId===e.userId)&&(e.isBot||Ot(e,this.myId)),o=!e.isBot&&ue(e)==="playing"&&jt(e,this.tableId)&&t.has(e.tableId),l=Re(e.meta?.origin??"",e.ruleType??"",ue(e)),a=s?c`<button class="btn-chat" aria-label="Unread message from ${e.userName}" @click=${()=>Q(this,"open-chat",e.userId)}>💬</button>`:o?c`<button class="btn-spectate" aria-label="Spectate ${e.userName}'s game" @click=${()=>Q(this,"spectate",e)}>Spectate</button>`:n?c`<button class="btn-challenge" aria-label="Challenge ${e.userName}" ?disabled=${this.isChallengePending} @click=${()=>C?window.location.href="https://billiards.tailuge.workers.dev/lobby":Q(this,"challenge",e.userId)}>Challenge</button>`:c``;return c`
            <li aria-label="${e.userName}">
                <div class="user-info">
                    <span class="user-name" @click=${()=>Q(this,"open-chat",e.userId)} style="cursor: pointer"><span title="${_(e.meta?.country).title}">${_(e.meta?.country).emoji}</span> ${e.userName} <span aria-label="${l.title}" role="img">${l.emoji}</span></span>
                </div>
