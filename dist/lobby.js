    ...
    `;constructor(){super(),this.limit=Ls,this._leaderboard=[],this._loaded=!1}async connectedCallback(){super.connectedCallback();let e=await Ke();e&&(this._leaderboard=Os(e),this._loaded=!0)}render(){let e=this._leaderboard.slice(0,Math.max(1,this.limit));return o`
        ...
        `}};customElements.define("trophy-cabinet",Qe);var Xe=class extends S{static styles=Dt;connectedCallback(){if(super.connectedCallback(),I){this.classList.add("loaded");return}let e=this._fontsReady(),t=()=>e.then(()=>{this.classList.add("loaded"),this.requestUpdate()});fetch(`${j}/api/summary`,{mode:"cors"}).then(s=>s.json()).then(s=>{this._data=s,t()}).catch(()=>{this._err=!0,t()})}_fontsReady(){let e=document.fonts;return e?.load?Promise.all([e.load("200 1rem 'Exo'"),e.load("600 1rem 'Exo'")]).catch(()=>{}):Promise.resolve()}render(){return I?o`<span class="loading">We have moved <a href="https://billiards.tailuge.workers.dev/lobby">Play online here</a></span>`:o`
            ...
            <tr><th>Name</th><th></th></tr>
                ${e[s].slice(0,4).map((i,n)=>o`<tr><td>${Je(n)} ${i.name}</td><td><replay-button url="${qe(`${j}/api/rank/${i.id}?ruletype=${s}&lod=${u.lod}`,u.clientId,u.userName)}" label="${i.score}"></replay-button></td></tr>`)}
            </table></div>
                    ...
                    <div class="group-body">
                        ...
                        <tr><th>Rule</th><th>Match</th><th>Ago</th><th class="city-col">City</th><th></th></tr>
                            ${s.map(n=>o`<tr>
                                ...
                                <td class="ago">${Gt(n.timestamp)}</td>
                                <td class="city-col">${n.locationCity??""}</td>
<td class="replay-col">
                                    ${n.hasReplay?o`<replay-button prefix="${w(n.locationCountry).emoji}" prefixTitle="${w(n.locationCountry).title}" url="${qe(`${j}/api/match-replay?id=${n.id}&lod=${u.lod}`,u.clientId,u.userName)}"></replay-button>`:w(n.locationCountry).emoji}
                                </td>
                            </tr>`)}
                        </table></div>
                    </div>
    ...
    `];_renderStatus(e){return o`<span aria-label="${e.title}" role="img">${e.emoji}</span>`}async autoExpandIfSupported(){if(this.#e)return;if((this.slots||[]).filter(t=>t.status==="online").length>4){await this.updateComplete;let t=this.renderRoot.querySelector(".expand-toggle");t&&getComputedStyle(t).visibility!=="hidden"&&(this.#e=!0,q(this,"user-list-toggle",{expanded:this.#e}),this.requestUpdate())}}updated(){if(!this.#e)return;let e=this.renderRoot.querySelector("ul");if(!e)return;let t=e.scrollHeight;t>0&&e.style.setProperty("--ul-expanded-height",t+"px")}#s(){this.#e=!this.#e,q(this,"user-list-toggle",{expanded:this.#e}),this.requestUpdate()}render(){let e=this.slots||[];if(e.filter(i=>i.status==="online").length===0)return o`<div class="empty">No other players online yet. Invite a friend!</div>`;let s=new Set(_e(this.users||[]).filter(i=>i.players.length>1).map(i=>i.tableId));return o`
            ...
            <div class="expand-toggle" @click=${this.#s}>
                ...
                </li>`}return this._row(e.user,s)}_row(e,t){let s=this.pendingChats?.get(e.userId)>0,n=!(this.challenges?.[e.userId]?.challengerId===e.userId)&&(e.isBot||ts(e,this.myId)),a=!e.isBot&&ve(e)==="playing"&&ss(e,this.tableId)&&t.has(e.tableId),c=De(e.meta?.origin??"",e.ruleType??"",ve(e),e.options,e.arenaId),l=s?o`<button class="btn-chat" aria-label="Unread message from ${e.userName}" @click=${()=>q(this,"open-chat",e.userId)}>💬</button>`:a?o`<button class="btn-spectate" aria-label="Spectate ${e.userName}'s game" @click=${()=>q(this,"spectate",e)}>Spectate</button>`:n?o`<button class="btn-challenge" aria-label="Challenge ${e.userName}" ?disabled=${this.isChallengePending} @click=${()=>I?window.location.href="https://billiards.tailuge.workers.dev/lobby":q(this,"challenge",e.userId)}>Challenge</button>`:o``;return o`
            <li aria-label="${e.userName}">
                <div class="user-info">
                    <span class="user-name" @click=${()=>q(this,"open-chat",e.userId)} style="cursor: pointer"><span title="${w(e.meta?.country).title}">${w(e.meta?.country).emoji}</span> <span class="name-wrap">${e.userName}<span class="loc-tip">${e.meta?.city??""}</span></span> ${this._renderStatus(c)}</span>
                </div>
                <div class="actions">${l}</div>
            </li>`}};customElements.define("user-list",tt);var Ce=(r,e,t)=>r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0})),st=class extends f{static properties={lobby:{type:Object},targetId:{type:String},targetName:{type:String},_messages:{state:!0},_unread:{state:!0}};static styles=[v,D,b`
    ...
    `];constructor(){super(),this._messages=new Map,this._unread=new Map,this._lobbyBound=!1}willUpdate(e){e.has("lobby")&&this.lobby&&!this._lobbyBound&&(this._lobbyBound=!0,this.lobby.onChat(t=>{let s=t.senderId,i=[...this._messages.get(s)??[],t];if(this._messages=new Map(this._messages).set(s,i),s!==this.targetId){let n=(this._unread.get(s)??0)+1;this._unread=new Map(this._unread).set(s,n),Ce(this,"unread-changed",{userId:s,count:n})}this.requestUpdate()})),e.has("targetId")&&this.targetId&&this._unread.has(this.targetId)&&(this._unread=new Map(this._unread).set(this.targetId,0),Ce(this,"unread-changed",{userId:this.targetId,count:0}))}_send(e){e.preventDefault();let t=this.shadowRoot.querySelector("input"),s=t.value.trim();if(!s||!this.lobby||!this.targetId)return;this.lobby.sendChat(this.targetId,s);let n={messageType:"chat",senderId:this.lobby.currentUser.userId,recipientId:this.targetId,text:s},a=[...this._messages.get(this.targetId)??[],n];this._messages=new Map(this._messages).set(this.targetId,a),t.value="",this.requestUpdate()}updated(e){if(e.has("targetId")){let t=this.shadowRoot.querySelector(".thread");t&&(t.scrollTop=t.scrollHeight);let s=this.shadowRoot.querySelector("input");s&&s.focus()}else if(e.has("_messages")){let t=e.get("_messages");if(this._messages.get(this.targetId)!==t?.get(this.targetId)){let s=this.shadowRoot.querySelector(".thread");s&&(s.scrollTop=s.scrollHeight)}}}render(){if(!this.targetId)return o``;let e=this.lobby?.currentUser?.userId,t=this._messages.get(this.targetId)??[];return o`
            ...
            </div>`}};customElements.define("message-modal",st);var Te=(r,e,t)=>r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0})),Bs={raceTo:"Race to",reds:"Reds",shotClock:"Shot clock",collaboration:"Collaboration",practice:"Practice"},ps=(r,e)=>r?Object.entries(r).filter(([,t])=>!(typeof t=="boolean"&&t===!1)).map(([t,s])=>{if(t.startsWith("handicap_")){let n=t.slice(9),a=e&&n===e?"Your handicap":"Handicap";return typeof s=="boolean"?a:`${a}: ${s}`}let i=Bs[t]??t;return typeof s=="boolean"?i:`${i}: ${s}`}):[],it=class extends f{static properties={challenge:{type:Object},sent:{type:Object},myId:{type:String}};static styles=[v,Ot,zt];render(){return this.challenge?this._incoming(this.challenge):this.sent?this._sent(this.sent):o``}_incoming(e){let t={...e.options};if(Object.keys(t).some(i=>i.startsWith("handicap_"))){let i=localStorage.getItem(`handicap_${e.ruleType}`)||"15";t["handicap_"+this.myId]=i}let s=ps(t,this.myId);return o`
            <div class="banner">
            ...
            </div>`}};customElements.define("user-badge",at);var Fs=`https://${O}/api/stats`,ot=class extends f{static styles=b`
    ...
    `];constructor(){super(),this._open=!1,this._showStats=!1,this._copied=!1,this._picker=null,this._theme=document.documentElement.getAttribute("theme")||"light",this._notifEnabled=typeof Notification<"u"&&Notification.permission==="granted",this._onKeydown=this._onKeydown.bind(this),this._onMessage=this._onMessage.bind(this)}connectedCallback(){super.connectedCallback(),window.addEventListener("keydown",this._onKeydown),window.addEventListener("message",this._onMessage)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("keydown",this._onKeydown),window.removeEventListener("message",this._onMessage)}_onMessage(e){e.data?.type==="done"&&(this._picker=null)}_onKeydown(e){e.key==="Escape"&&(this._picker?this._picker=null:this._open&&this._close())}_toggle(e){e.stopPropagation(),this._open=!this._open}_close(){this._open=!1,this._picker=null}_setTheme(e){let t=e.target.checked?"dark":"light";this._theme=t,document.documentElement.setAttribute("theme",t),localStorage.setItem("theme",t),this.dispatchEvent(new CustomEvent("theme-changed",{detail:t,bubbles:!0,composed:!0}))}_share(){navigator.share&&/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)?navigator.share({title:document.title,url:location.href}):navigator.clipboard.writeText(location.href).then(()=>{this._copied=!0,setTimeout(()=>{this._copied=!1},2e3)})}async _toggleNotifications(e){if(e.target.checked&&typeof Notification<"u"){let t=await Notification.requestPermission();this._notifEnabled=t==="granted"}else this._notifEnabled=!1;this.requestUpdate()}render(){return o`
            ...
            ${this._open?o`
                ...
                `:""}`:""}
        `}};customElements.define("settings-modal",lt);var Js=3e4,qs=5,gs=1800*1e3,Gs=r=>(Math.floor(r/gs)+1)*gs,bs=[{name:"Nine Ball Mini Hourly Arena",ruleType:"nineball",options:{tableSize:"6",freeaim:"true"}},{name:"Eight Ball Mini Hourly Arena",ruleType:"eightball",options:{tableSize:"6",freeaim:"true"}},{name:"Snooker Mini Hourly Arena",ruleType:"snooker",options:{tableSize:"6",reds:"3",freeaim:"true"}}],Ys=b`
...
`,Vs=(r,e=!1,t=null,s=!0)=>{let i=`lobby.html?tournamentId=${encodeURIComponent(r.id)}`,n=a=>{t?(a.preventDefault(),a.stopPropagation(),t(r.id)):a.currentTarget instanceof HTMLButtonElement&&(a.preventDefault(),window.location.href=a.currentTarget.closest("a").href)};return o`<a class="arena-item ${e?"completed":""}" href=${i} @click=${n}>
    ...
    </a>`},ct=class extends f{static properties={heading:{type:String},_arenas:{state:!0},_error:{state:!0},selectable:{type:Boolean}};static styles=[Ys,b`
        :host { display: block; }
