...
<td class="replay-col">
    ...
    `];render(){let e=(this.users||[]).filter(t=>t.userId!==this.myId);return e.length===0?l`<div class="empty">No other players online yet. Invite a friend!</div>`:l`<ul aria-label="Online players">${Ht(e,t=>t.userId,t=>this._row(t))}</ul>`}_row(e){let t=this.pendingChats?.get(e.userId)>0,i=!(this.challenges?.[e.userId]?.challengerId===e.userId)&&(e.isBot||jt(e,this.myId)),n=!e.isBot&&Be(e)==="playing"&&Bt(e,this.tableId),o=At(e.meta?.origin??"",e.ruleType??"",Be(e)),c=t?l`<button class="btn-chat" aria-label="Unread message from ${e.userName}" @click=${()=>L(this,"open-chat",e.userId)}>💬</button>`:n?l`<button class="btn-spectate" aria-label="Spectate ${e.userName}'s game" @click=${()=>L(this,"spectate",e)}>Spectate</button>`:i?l`<button class="btn-challenge" aria-label="Challenge ${e.userName}" ?disabled=${this.isChallengePending} @click=${()=>C?window.location.href="https://billiards.tailuge.workers.dev/lobby":L(this,"challenge",e.userId)}>Challenge</button>`:l``;return l`
            ...
            </div>`}};customElements.define("user-badge",Fe);var as=`https://${D}/api/stats`,We=class extends f{static styles=b`
