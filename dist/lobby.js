                ...
                <div class="group-body">
                    ${i.map(n=>l`
                        ...
                        <tr><th>Name</th><th></th></tr>
                            ${e[n].slice(0,4).map((o,c)=>l`<tr><td>${Ne(c)} ${o.name}</td><td><replay-button url="${Me(`${R}/api/rank/${o.id}?ruletype=${n}&lod=${u.lod}`,u.clientId,u.userName)}" label="${o.score}"></replay-button></td></tr>`)}
                        </table></div>
                    `)}
                </div>
            </div>
...
<td class="replay-col">
                                    ${n.hasReplay?l`<replay-button prefix="${C(n.locationCountry).emoji}" prefixTitle="${C(n.locationCountry).title}" url="${Me(`${R}/api/match-replay?id=${n.id}&lod=${u.lod}`,u.clientId,u.userName)}"></replay-button>`:C(n.locationCountry).emoji}
                                </td>
                            </tr>`)}
                        </table></div>
                    </div>
                            ...
                            <div class="tbl"><table><caption><a href="${R}/elo" target="_blank" rel="noopener">${O(n)} <span style="font-size:0.75rem;font-weight:200">Rankings</span></a></caption>
                             <tr><th>Name</th><th>Score</th><th>W</th><th>L</th></tr>
                                 ${t[n].slice(0,4).map((o,c)=>l`<tr>
                                     <td><a href="${R}/player/${encodeURIComponent(o.name)}?ruleType=${n}">${Ne(c)} ${o.name}</a></td>
