...
describe("LobbyIndicator", () => {
  ...
  it("does not log lobby chat to lobby logs", async () => {
    const { NetworkLogger } = require("../../src/utils/network-logger")
    // Clear previous logs
    ;(NetworkLogger as any).lobbyLogs = []
    ...
    const mockLobby = (indicator as any).lobby
    const onChatCallback = mockLobby.onChat.mock.calls[0][0]

    // Simulate receiving a lobby chat
    onChatCallback({
      senderId: "u2",
      text: "hello world",
    })

    const lobbyLogs = NetworkLogger.getLobbyLogs()
    const chatLogs = lobbyLogs.filter((log: any) => log.label === "chat")
    expect(chatLogs).toHaveLength(0)

    await indicator.stop()
  })

  it("does not log AIM, STARTAIM, WATCHAIM, or CHAT events to game logs", () => {
    const { NetworkLogger } = require("../../src/utils/network-logger")
    const { logNetEvent } = require("../../src/utils/event-log")
    const { EventType } = require("../../src/events/eventtype")

    // Clear previous logs
    ;(NetworkLogger as any).gameLogs = []
    ...
    logNetEvent(
      "Player 1",
      { type: EventType.AIM, clientId: "p1" } as any,
      "broadcast"
    )
    logNetEvent(
      "Player 1",
      { type: EventType.STARTAIM, clientId: "p1" } as any,
      "broadcast"
    )
    logNetEvent(
      "Player 1",
      { type: EventType.WATCHAIM, clientId: "p1" } as any,
      "broadcast"
    )
    logNetEvent(
      "Player 1",
      { type: EventType.CHAT, clientId: "p1" } as any,
      "broadcast"
    )

    // Log a non-ignored event type
    logNetEvent(
      "Player 1",
      { type: EventType.HIT, clientId: "p1" } as any,
      "broadcast"
    )

    const gameLogs = NetworkLogger.getGameLogs()
    const logLabels = gameLogs.map((log: any) => log.label)

    expect(logLabels).not.toContain("net: Player 1 broadcast AIM : p1")
    expect(logLabels).not.toContain("net: Player 1 broadcast STARTAIM : p1")
    expect(logLabels).not.toContain("net: Player 1 broadcast WATCHAIM : p1")
    expect(logLabels).not.toContain("net: Player 1 broadcast CHAT : p1")
    expect(logLabels).toContain("net: Player 1 broadcast HIT : p1")
  })
})
