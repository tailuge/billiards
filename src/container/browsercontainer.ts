...
export class BrowserContainer {
  ...
  constructor(canvas3d, params) {
    ...
    this.resumeHandler = new ResumeHandler(this)
  }
  ...
  netEvent(e: string, msgId?: string) {
    ...
    if (this.resumeHandler.shouldDropMessage(msgId)) {
      return
    }
    ...
    if (this.resumeHandler.shouldTrace()) {
      this.resumeHandler.traceEvent(event)
    }
