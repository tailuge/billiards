export class RealPosition {
    ...
    private interpolateUntilMove(data: any, delta: number = 0.3, epsilon: number = 0.00001): any {
    ...
    private interpolateAllBalls(shot: any): any {
        shot.balls = Object.fromEntries(
            Object.entries(shot.balls)
                .map(([num, data]) => [num, this.interpolateUntilMove(data)])
        );
        return shot;
    }

    public getPositionsAtTime(shotId: number, time: number): { [key: string]: { x: number; y: number } } | null {
        const shot = this.shots.find(s => s.shotID === shotId);

        const ballPositions: { [key: string]: { x: number; y: number } } = {};

        const getInterpolatedPosition = (times: number[], positions: number[], targetTime: number): number => {
